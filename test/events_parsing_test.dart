...
// ignore: long-method
void main() {
  group('PusherChannelsReadEvent', () {
    test(
      'getters name and data must be respective to the one from rootObject',
      () {
        final event = jsonEncode({
          'event': 'hello',
          'data': jsonEncode({'payload': 3}),
        });
        final readEvent = PusherChannelsReadEvent.tryParseFromDynamic(event);
  ...
  group('PusherChannelsPongEvent |', () {
    test(
      ...
      () {
        ...
        final stringMessage = jsonEncode(mapMessage);
        ...
        final fromMap = PusherChannelsPongEvent.tryParseFromDynamic(mapMessage);
        ...
        expect(mustBeNull, null);
        expect(fromMap, isA<PusherChannelsPongEvent>());
        expect(fromString, isA<PusherChannelsPongEvent>());
        expect(withWrongName, null);
      },
    );
  });
  group('PusherChannelsPingEvent |', () {
    test(
      'tryParseFromDynamic gives null if message not String or Map, if wrong event name',
      () {
        final mapMessage = {
        ...
        final fromMap = PusherChannelsPingEvent.tryParseFromDynamic(mapMessage);
        ...
        expect(mustBeNull, null);
        expect(fromMap, isA<PusherChannelsPingEvent>());
        expect(fromString, isA<PusherChannelsPingEvent>());
        expect(withWrongName, null);
      },
    );
  });
  group('PusherChannelsErrorEvent |', () {
    test('testing members of PusherChannelsErrorEvent', () {
      final mapMessage = {
        'event': 'pusher:error',
        'data': jsonEncode(<String, String>{'code': '123', 'message': 'hello'}),
      };
      final event = PusherChannelsErrorEvent.tryParseFromDynamic(mapMessage);

      expect(event?.name, 'pusher:error');

      expect(event?.code, 123);
    ...
    test(
      ...
      () {
        ...
        expect(mustBeNull, null);
        expect(fromMap, isA<PusherChannelsErrorEvent>());
        expect(fromString, isA<PusherChannelsErrorEvent>());
        expect(withWrongName, null);
      },
    );
  });
  group('PusherChannelsConnectionEstablishedEvent |', () {
    test('testing members of PusherChannelsConnectionEstablishedEvent', () {
      final mapMessage = {
      ...
      expect(event?.activityTimeoutDuration?.inSeconds, 12);
    ...
    test(
      ...
      () {
        final mapMessage = {
          'event': 'pusher:connection_established',
          'data': jsonEncode(<String, dynamic>{'socket_id': 12}),
        };
        ...
        expect(mustBeNull, null);
        expect(fromMap, isA<PusherChannelsConnectionEstablishedEvent>());
        expect(fromString, isA<PusherChannelsConnectionEstablishedEvent>());
        expect(withWrongName, null);
        expect(withoudSocketId, null);
        expect(withWrongData, null);
      },
    );
  });
}
